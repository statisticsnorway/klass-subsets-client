    ...
    return (
        <div className='page'>
            {!subset ? (
                ...
                <div>
                    <Title size={3}>
                        {subset.name?.find(name => name.languageCode === "nb")
                            ?.languageText || "no title"}
                    </Title>
                    <p style={{ fontSize: "calc(10px + 0.3vmin)" }}>
                        ID: <strong>{subset?.id || "-"} </strong>
                        {t("Version")}:{" "}
                        <strong>{subset.version || "-"} </strong>
                        {t("Updated")}:{" "}
                        <strong>{subset.lastUpdatedDate || "-"} </strong>
                        {t("Status")}:{" "}
                        <strong>{subset.administrativeStatus || "-"} </strong>
                    </p>
                    <Paragraph style={{ fontSize: "calc(10px + 0.8vmin)" }}>
                        {subset.description?.find(
                            desc => desc.languageCode === "nb"
                        )?.languageText || t("No description")}
                    </Paragraph>

                    <Paragraph>
                        <strong>{t("Owner")}:</strong> {subset.createdBy || "-"}
                    </Paragraph>
                    <Paragraph>
                    ...
                    <Paragraph>
                        ...
                        {subset.administrativeDetails.find(
...
export const SubsetPreview = ({ subset }) => {
    ...
    return (
        <>
            <Title size={2}>
                {subset.name[0]?.languageText ||
                    t("Subset has got no title yet")}
            </Title>
            <Paragraph>
                <strong>ID:</strong> {subset.id || "-"}
            </Paragraph>
            <Paragraph>
            ...
            <Paragraph>
                {subset.description[0]?.languageText || t("No description")}
            </Paragraph>
...
export const Code = ({ code }) => {
    ...
    return (
        <Accordion header={code.name} subHeader={code.code || t("Code")}>
            <p>
                <strong>{t("Short name")}:</strong> {code.shortName || "-"}
            </p>
            <p>
            ...
            <p>
                <strong>{t("Level")}:</strong> {code.level}
            </p>
            {code.parentCode && (
                <p>
                    <strong>{t("Parent code")}:</strong> {code.parentCode}
                </p>
            )}
            <p>
            ...
            {!code.notes ? (
                ...
                code.notes.map(note => (
                    <div style={{ padding: "5px 25px 10px 25px" }}>
                        ...
                        />
                        <Text small>
                            <strong>«{note.versionName}»</strong> ({t("valid")}:{" "}
                            {note.validFrom || "..."} - {note.validTo || "..."})
                        </Text>
                    </div>
                ))
            )}
...
export const SubsetBanner = ({ subset }) => {
    ...
    return (
        <div style={{ lineHeight: "50%", margin: "50px 0" }}>
            ...
            <p style={{ fontSize: "calc(10px + 0.8vmin)" }}>
                {subset?.description?.find(
                    description => description.languageCode === "nb"
                )?.languageText || t("No description")}
            </p>
        </div>
    );
