    ...
    useEffect(() => {
        if (codes && codesWithNotes) {
            item.codes.forEach(
                code =>
                    (code.notes =
                        codesWithNotes.find(c => code.name === c.name)?.notes ||
                        [])
            );
        }
    ...
    return (
        <>
            <div style={{ display: "flex" }}>
                ...
                <button
                    onClick={() => item.error && setExpander(toggle.alert())}
                >
                ...
                <button onClick={() => setExpander(toggle.codes())}>
                    <ListIcon
                        color={check.hasCodes() ? "#3396D2" : "#C3DCDC"}
                    />
                </button>
            ...
            {expander.showCannot && (
                <div style={{ backgroundColor: "#ece6fe" }} className='panel'>
                    <Text>
                        {t(
                            "Code list cannot be added to the subset due to lack of codes"
                        )}
                    </Text>
                </div>
            )}
...
export const Codes = ({ from, to, codes = [], id, include }) => {
    ...
    const [renderedCodes, setRenderedCodes] = useState(
        codes.slice(0, Math.min(codesToLoadFirstRender, codes.length))
    );
                    ...
                    <>
                        <div style={{ padding: "5px" }}>
                            ...
                            <button
                                onClick={() => {
                                    codes.forEach(
                                        code => (code.included = !code.included)
                                    );
    ...
    return (
        <>
            <div style={{ display: "flex" }}>
                ...
                >
                    <MoreHorizontal
                        color={item.notes?.length > 0 ? "#62919A" : "#C3DCDC"}
                    />
                </button>
            </div>
                            ...
                            >
                                ...
                                <div
                                    ...
                                    dangerouslySetInnerHTML={{
                                        __html: replaceRefWithHTMLAndSanitize(
                                            note.note
                                        )
                                    }}
                                />
                                <Text small>
                                    <strong>«{note.versionName}»</strong> (
                                    {t("valid")}: {note.validFrom || "..."} -{" "}
                                    {note.validTo || "..."})
                                </Text>
                            </div>
                        ))
...
export const CodelistInfo = ({ id, info }) => {
    ...
    return (
        <div style={{ backgroundColor: "#eff4f5" }} className='panel'>
            ...
            <Paragraph>
                <strong>{t("Description")}:</strong> {info.description || "-"}
            </Paragraph>
        </div>
    );
};
